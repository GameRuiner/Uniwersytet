...
(define (make-node elem heap-a heap-b)
  ;;; ZZZ: filled in the implementation
  (define rank-a (rank heap-a))
  (define rank-b (rank heap-b))
  (if (< rank-a rank-b)
      (let((rank-e (inc rank-a))
           (right heap-a)
           (left heap-b))
        (list 'hnode elem rank-e left right))
      (let((rank-e (inc rank-b))
           (right heap-b)
           (left heap-a))
        (list 'hnode elem rank-e left right))))
...
(define (heap-merge h1 h2)
  (cond
   ...
   [else (if (< (elem-priority (node-elem h1)) (elem-priority (node-elem h2)))
              (make-node (node-elem h1)
                         (heap-merge (node-right h1) h2)
                         (node-left h1))
              (make-node (node-elem h2)
...
(define xs1 (heapsort(randlist 100 100)))
(sorted? xs1)
(define xs2 (heapsort(randlist 100 100)))
(sorted? xs2)
(define xs3 (heapsort(randlist 100 100)))
(sorted? xs3)
