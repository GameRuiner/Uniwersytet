...
(define (clause-trivial? c)
  (if (null? (res-clause-pos c))
      false
      (if (member (car (res-clause-pos c)) (res-clause-neg c))
          true
          (clause-trivial? (res-clause (cdr (res-clause-pos c)) (res-clause-neg c) (res-clause-proof c))))))
...
(define (resolve c1 c2) 
  ;; TODO: zaimplementuj!;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
  (if (eq-list? (res-clause-pos c1) (res-clause-neg c2) )
      false
      (let (( p (find-p (res-clause-pos c1) (res-clause-neg c2))))
        (res-clause
         (remove-duplicates-vars (merge-vars (without-p p (res-clause-pos c1)) (res-clause-pos c2)))
         (remove-duplicates-vars (merge-vars (without-p p (res-clause-neg c2)) (res-clause-neg c1)))
         (proof-res p (fifth c1) (fifth c2))))))
...
(define (generate-valuation resolved)
  ...
  (print (res-clause-pos (first resolved)))
  (list 'sat (res-clause-pos (first resolved))))
...
(define c1
  (res-clause '(a b c) '(x) (proof-axiom '((literal false 'x) (literal true 'a) (literal true 'b) (literal true 'c)))))


(define c2
  (res-clause '(b d) '(c) (proof-axiom '((literal false 'c) (literal true 'b) (literal true 'd)))))
