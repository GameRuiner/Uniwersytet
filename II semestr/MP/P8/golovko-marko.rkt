...
(eval
         '(let (op-e (lambda (x) (car x)))
           (let (args-e (lambda (x) (cdr x)) )
             (let (left (lambda (x) (car x)))
               (let (right (lambda (x) (cdr x)))
      (let (eval-arith (lambda-rec (ar e)
                          (cond [(eq? (op-e e) 'n) (args-e e)]
                                [(eq? (op-e e) '+) (+ (ar (left (args-e e))) (ar (right (args-e e))))]
                                [(eq? (op-e e) '-) (- (ar (left (args-e e))) (ar (right (args-e e))))]
                                [(eq? (op-e e) '*) (* (ar (left (args-e e))) (ar (right (args-e e))))]
                                [(eq? (op-e e) '/) (/ (ar (left (args-e e))) (ar (right (args-e e))))]
                                ))) 
        ...
        (eval-arith (cons '+
                          (cons
                           (cons '* (cons (cons 'n 6) (cons 'n 2)))
                           (cons '/ (cons (cons 'n 16) (cons 'n 8))))))))))))
