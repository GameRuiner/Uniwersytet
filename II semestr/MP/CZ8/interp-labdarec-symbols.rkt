...
;;w 2
#|(eval '(let (rev (lambda(x)
          (let (rer-h (lambda-rec (f xs ys)
                                    (if (null? xs)
                                        ys
...
#|
(eval
 '(let (apply (lambda (f a)
                     (let (app-rec (lambda-rec (app f x)
                                               (cons (f (car x)) (app (cdr x)))
                                               (app-rec f a))))))
         (let (op-e (lambda (e)
                      (cadr e)))
           (let (args-e (lambda (e)
                      (caddr e)))
      (let (eval-arith (lambda (x)
            (let (rec-a   (lambda-rec (f arith-e)
                          ((op-e arith-e) (apply f (args-e arith-e)))
                          (rec-a x ))))))
        (eval-arith (+ 5 (- 6 7)))))))
   )
|#
(eval '(let (aux (lambda(x)
                   (let (raux (lambda-rec (f n)
                            (if (= n 1)
                                1
                                (* n (f (- n 1))))
                            (raux x))))))
