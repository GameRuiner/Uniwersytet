...
(define arith-grammar
  (append num-grammar
     '([add-expr {ADD-MANY   sub-expr (token #\+) add-expr}
                 {ADD-SINGLE sub-expr}]
       [sub-expr {SUB-MANY div-expr (token #\-) sub-expr}
                 {SUB-SINGLE div-expr}]
       [div-expr {DIV-MANY   mult-expr (token #\/) div-expr}
                 {DIV-SINGLE mult-expr}]
  ...
  (cond [(eq? (node-name t) 'SUB-SINGLE)
        ...
        [(eq? (node-name t) 'DIV-MANY)
         ...
         (binop-cons
          '/
          (arith-walk-tree (second t))
          (arith-walk-tree (second (fourth t)))))]
...
(define (aux2 t binop) ;вытянет следующее действие после вычитания
 (cond [(eq? (node-name t) 'ADD-MANY)
         (binop-cons
          '+
          binop
          (arith-walk-tree (second t)))]
...
;;TEST
(display "2-2-2-2-2-2-2 = ")
(calc "2-2-2-2-2-2-2")
(display "1024/2/2/2/2/2 = ")
(calc "1024/2/2/2/2/2")
(display "(2/2-(3+2))*(8/(6-4)) = ")
(calc "(2/2-(3+2))*(8/(6-4))")
(display "2-2/2 = ")
(calc "2-2/2")