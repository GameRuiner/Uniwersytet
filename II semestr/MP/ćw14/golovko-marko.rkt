...
;(define (make-librarian name birthplace threshold)
...
(define biblioteka (make-place 'biblioteka))
(define toaleta (make-place 'toaleta))
(define wybrzeże (make-place 'wybrzeże))
(define matematyka (make-place 'matematyka))
...
(can-go-both-ways hol 'na-zewnątrz 'wewnątrz wybrzeże)
(can-go-both-ways wybrzeże 'pólnoc 'południe matematyka)
(can-go-both-ways piętro-wschód 'wschód 'zachód toaleta)
(can-go-both-ways matematyka 'góra 'dól biblioteka)
...
(define mgo       (make&install-person 'mgo toaleta 0))
(define bibliotekarz       (make&install-person 'mgo biblioteka 0))
(define złodziej (make&install-person 'złodziej wybrzeże 3))
...
(define książki (make&install-thing 'książki biblioteka))
(define rower (make&install-thing 'rower wybrzeże))
(define woda (make&install-thing 'woda toaleta))

;; TODO: dodatkowe osoby i przedmioty
(ask złodziej 'take rower)
